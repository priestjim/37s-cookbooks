...
if ( is_array($g_metrics) && is_array($g_metrics_group) )
   {
      ...
      foreach ( $g_metrics_group as $group => $metric_array )
	    ...
	    if ( $group == "" ) {
            ...
            if ( $count > 0 )
              $tpl->assign("group_metric_count", ($count));
            else
	    ...
	    $i = 0;
            ...
            foreach ( $g_metrics as $name => $v )
               {
		if ( ! preg_match ("/MYSQL/i", $name) ) {
		if ( ! preg_match ("/memcache/i", $name) ) {
		if ( ! preg_match ("/haproxy/i", $name) ) {
      ...
      foreach ( $g_metrics_group as $group => $metric_array )
         {
            ...
            $tpl->newBlock("vol_group_info");
            $tpl->assign("group", $group);
            $tpl->assign("show", "style='display:none'");
            $count = 0;
	    foreach ( $g_metrics as $name => $v )
	    {
  		if ( preg_match ("/MYSQL/i", $name) ) {
